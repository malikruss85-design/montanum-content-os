...
test('reports queued, narration-ready, and final-render-ready events to Make', async () => {
  ...
  try {
    const app = createApp({ ...process.env, MPE_DATA_DIR: path.join(temp, 'data'), MPE_LOG_DIR: path.join(temp, 'logs'), MPE_OUTPUT_DIR: path.join(temp, 'out'), MPE_CALLBACK_URL: 'https://make.example.test/hook', MPE_CALLBACK_TOKEN: 'callback-token', MPE_CALLBACK_SIGNING_SECRET: 'callback-secret' });
