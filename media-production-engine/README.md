...
```powershell
'{"narrationPublicId":"reels/rec123/v1/narration","subtitlesPublicId":"reels/rec123/v1/subtitles.en","scenes":[...]}' |
  npm run plan:cloudinary
```

Every scene must reference a durable `https://res.cloudinary.com/.../upload/...` URL, use the approved `sequence`, and include either `stillDuration` (photo/render) or `trimStart`/`trimEnd` (video). Include the approved `subtitleSegment` for each scene to produce an SRT suitable for Cloudinary's `l_subtitles` layer.
