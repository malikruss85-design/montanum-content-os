...
| 2 | Airtable: Update a Record | Set `Reel Production Status = Narration Generating`; clear `Reel Production Error`. | Locks the record against another scheduled pass. |
| 3 | Airtable: Search Records | Read the record's linked Bundle Media in `Order` order. Fail if no source is a `res.cloudinary.com` delivery URL. | On failure: `Production Failed` with safe summary. |
| 4 | OpenAI: Generate speech from text | Existing `My OpenAI connection`; model `gpt-4o-mini-tts`; voice `coral`; response format MP3; text = `Voice-over Script`. | No Airtable write yet. |
| 5 | Cloudinary: Upload a resource | Existing `My Cloudinary connection`; upload the binary from #4 as resource type `video`; public ID `reels/<Content record ID>/v<Reel Brief Version>/narration`. | Store its secure URL in `Narration Asset`. |
